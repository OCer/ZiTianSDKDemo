...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath  // 当用户点击某个表格时调用
{
    ...
    [[ZTAdLoadTask sharedManager] showAdWithAgent:ZTAdAgentTypeBUD type:type positionName:nil showBlock:^(NSError *error) {
        if (error)
        {
            NSLog(@"广告显示失败 = %@", error);
        }
...
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    if (self = [super initWithFrame:frame style:style])
    {
        ...
        [self setData:[NSMutableArray arrayWithObjects:@"插屏", @"banner", @"激励视频", @"开屏", nil]];
    }
